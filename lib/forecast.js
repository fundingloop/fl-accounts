...
// buildForecast({ startingFloat, floatAsOfDate, deposits, bills, today, horizonMonths, extraEvents })
// -> { currentBalance, series: [{date, balance}...], lowest: {balance, date}, events }
//
// extraEvents is a minimal generic hook for callers that project their own
// future outflows/inflows (e.g. payroll) alongside bills/deposits: each item
// is {date: Date (UTC midnight), amount, kind, description}, already clipped
// to [today, horizon] by the caller. They are concatenated with bill/deposit
// events before the sort, so the series and lowest-point maths include them.
...
// ---- forecast summary (multi-entity dashboard panel) --------------------

// forecastSummary(forecast) -> { opening, income, expenses, payroll, tax,
// closing, other }. Pure aggregation over a buildForecast() result's
// `events` (each {date, amount, kind, ...}), not `series` (a running-balance
// walk, not per-kind totals):
//   opening  = forecast.currentBalance
//   income   = sum of POSITIVE event amounts with kind 'deposit'
//   expenses = abs sum of amounts with kind 'bill'
//   payroll  = abs sum of amounts with kind 'payroll_net'
//   tax      = abs sum of amounts with kinds 'payroll_ssf' + 'payroll_tds'
//   closing  = opening + sum of ALL event amounts (every kind, any sign) -
//              matches the final point of buildForecast's `series`.
// Any event whose kind is none of the above is "unknown": its amount still
// flows into `closing` (closing always reflects every event) but is also
// accumulated separately into `other` so callers can see what is uncounted
// in the named buckets above. Null/undefined forecast is safe (returns all
// zeros).
export function forecastSummary(forecast) {
  const opening = Number(forecast?.currentBalance) || 0;
  let income = 0;
  let expenses = 0;
  let payroll = 0;
  let tax = 0;
  ...
  for (const evt of forecast?.events || []) {
  ...
  return { opening, income, expenses, payroll, tax, closing: opening + netAll, other };
}
